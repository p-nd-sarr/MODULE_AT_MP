class Employer::ImmatriculationStructurePrivesController < Employer::ApplicationController
  ...
  def index
    ...
    if @immatriculation.nil?
      type_employeur = Admin::TypeEmployeur.find_by_code('PUB_PARA')
      ...
      @immatriculation.mobileNumber = current_user.telephone
      @immatriculation.email = current_user.email
    end
  ...
  def call_web_service_immatriculation
    ...
    client = Savon.client(wsdl: "#{WS_IMM_BASE_PATH}/ouaf/XAIApp/xaiserver/IMMAT2_INBOUND?wsdl",
    ...
    personne_contact = infos_contact()
    ...
    begin
      response = client.call(:IMMAT2_INBOUND) do
        message(input: {
    ...
    if !response.nil? and response.success?
      ...
      immatriculation.message = "Erreur sur le traitement. Merci de contacter l'administrateur"
      immatriculation.save!
    end
