class Employer::ImmatriculationIndividuellesController < Employer::ApplicationController
  ...
  def index
    ...
    if @immatriculation.nil?
      type_employeur = Admin::TypeEmployeur.find_by_code('INDV')
  ...
  def call_web_service_immatriculation
    ...
    client = Savon.client(wsdl: '#{WS_IMM_BASE_PATH}/ouaf/XAIApp/xaiserver/CM-CrtIndForXAI?WSDL',
    ...
    begin
      response = client.call(:CM - CrtIndForXAI) do
        message(input: {
  ...
  def get_infos
    immatriculation = current_user.immatriculation
    informations = {typeDemandeur: '', nom: '', prenom: '', typePieceIdentite: '', carteConsulaire: '', carteIdentiteNationale: '', passport: '',
                    nationalite: '', issueDate: '', expiryDate: '', dateOfBirth: '', country: '', cityOfBirth: '', landLineNumber: '',
                    mobileNumber: immatriculation.mobile_number, email: '', businessSector: '', mainLineOfBusiness: '', region: immatriculation.reprise_region.designation, department: immatriculation.reprise_departement.designation,
                    arondissement: immatriculation.reprise_ville.designation, commune: immatriculation.reprise_commune.designation, qartier: immatriculation.reprise_quartier.designation, address: adresse, tauxAt: '', zoneCSS: '', zoneIPRES: '', sectorCSS: '', sectorIPRES: '', agenceIPRES: ''}
