class Employer::ImmatriculationAssociationsController < Employer::ApplicationController
  ...
  def index
    ...
    if @immatriculation.nil?
      type_employeur = Admin::TypeEmployeur.find_by_code('ASSO')
    ...
    @salarie_immatriculation.type_mouvement = "Embauche"
  ...
  def immatriculation_params
    params.require(:immatriculation).permit(:type_immatriculation, :raison_sociale, :type_etablissement,
                                            :ninea, :ninet, :registre_commerce, :statut_juridique,
                                            :code_identification_fiscale, :date_immatriculation,
                                            :date_identification_fiscale, :date_identification_rc,
                                            :date_ouverture, :siege_social, :activite_principale,
                                            :secteur_activite, :email, :adresse, :mobileNumber,
                                            :region, :departement, :ville, :commune, :quartier,
                                            :sigle, :effectif_employe, :effectif_cadre,
                                            :embauche_employer, :embauche_cadre)
  end
  ...
  def call_web_service_immatriculation
    ...
    client = Savon.client(wsdl: "#{WS_IMM_BASE_PATH}/ouaf/XAIApp/xaiserver/IMMATRICULATION_INBOUND?wsdl",
                          endpoint: "#{WS_IMM_BASE_PATH}/ouaf/XAIApp/xaiserver/IMMATRICULATION_INBOUND",
    ...
    documents = {
        demande_ecrit: {delivered: true, url: ''}, form_demande: {delivered: true, url: ''},
    ...
    if !response.nil? and response.success?
      ...
      immatriculation.sectorIpres = response.to_hash[:immatriculation_inbound][:output][:sector_ipres]
      immatriculation.taux_at = response.to_hash[:immatriculation_inbound][:output][:taux_at]
      immatriculation.message = nil
