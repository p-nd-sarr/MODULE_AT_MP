class Employer::ImmatriculationDiplomatiquesController < Employer::ApplicationController
  ...
  def index
    ...
    if @immatriculation.nil?
      type_employeur = Admin::TypeEmployeur.find_by_code('REP_DIP')
  ...
  def call_web_service_immatriculation
    ...
    #client = Savon.client(wsdl: '#{WS_IMM_BASE_PATH}/ouaf/XAIApp/xaiserver/IMMAT_REP_DIPLO?wsdl',
    client = Savon.client(wsdl: '#{WS_IMM_BASE_PATH}/ouaf/XAIApp/xaiserver/IMMAT_REP_DIPLO?wsdl',
    ...
    infos_representant = get_personne_contact()

    infos_salaries = get_salaries_to_json()

    info_employeur = get_infos_employer()
    ...
    begin
      response = client.call(:IMMAT_REP_DIPLO) do
        message(input: {
            employerQuery: info_employeur,
            mainRegistrationForm: immatriculation.infos_etat_civil,
            legalRepresentativeForm: infos_representant,
            documents: documents,
  ...
  def get_infos_employer
    immatriculation = current_user.immatriculation

    informations = {regType: 'BVOLN', employerType: 'REP_DIP', estType: '', employerName: immatriculation.raison_sociale, nineaNumber: immatriculation.ninea}

    informations
  end

  def get_personne_contact
    representant = current_user.representant_legal

    informations = {recherche: '', nom: representant.last_name, prenom: representant.first_name, typeOfIdentity: representant.type_of_piece,
                    nin: representant.nin, identityIdNumber: representant.identity_number, ninCedeo: representant.ninCedeo,
                    telephoneFixe: representant.mobile_number, numeroMobile: representant.mobile_number, email: representant.email}
