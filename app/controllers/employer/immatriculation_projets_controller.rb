class Employer::ImmatriculationProjetsController < Employer::ApplicationController

  before_action :init_entity_list

  after_action :call_web_service_immatriculation, only: [:soumettre_demande]
  after_action :calcul_pourcentage, only: [:index]
  ...
  def index
    ...
    if @immatriculation.nil?
      type_employeur = Admin::TypeEmployeur.find_by_code('PROJ')
