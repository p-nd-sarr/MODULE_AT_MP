class Employer::ImmatriculationDomestiquesController < Employer::ApplicationController
  ...
  def index
    ...
    if @immatriculation.nil?
      ...
      type_employeur = Admin::TypeEmployeur.find_by_code('DOM')
  ...
  def create
    ...
    if @immatriculation.save
      redirect_to [:employer, 'immatriculations'], notice: "Immatriculation créée avec succès."
    else
  ...
  def valider_salarie
    ...
    redirect_to [:employer, 'immatriculations'], notice: 'Information sur les salariés validée.'
  end

  def valider_representant
    @immatriculation = current_user.immatriculation
    @immatriculation.representant_valide!
    redirect_to [:employer, 'immatriculations'], notice: 'Information sur le représentant validée.'
  end
  ...
  def valider_documents
    ...
    if @document_immatriculation.size > 0
      ...
      redirect_to [:employer, 'immatriculations'], notice: 'Information sur les documents validée.'
  ...
  def soumettre_demande
    ...
    redirect_to [:employer, 'immatriculations'], notice: 'La demande est soumise.'
  end
  ...
  def call_web_service_immatriculation
    ...
    #client = Savon.client(wsdl: '#{WS_IMM_BASE_PATH}/ouaf/XAIApp/xaiserver/inboundDomFrm?WSDL',
    client = Savon.client(wsdl: '#{WS_IMM_BASE_PATH}/ouaf/XAIApp/xaiserver/inboundDomFrm?WSDL',
    ...
    begin
      response = client.call(:immatriculation_inbound) do
        message(input: {
            registrationFormInfos: {receiveDate: '', documentLocator: ''},
            domesticRegistrationForm: immatriculation.infos_etat_civil,
  ...
  def info_domestiques

    representant = current_user.representant_legal
    immatriculation = current_user.immatriculation

    informations = {dateEmbauchePremierSalarie: immatriculation.embauche_employers, regType: 'BVOLN', idType: '', nin: representant.nin, lastName: representant.last_name, firstName: representant.first_name, ninCedeao: representant.nin_cedeao, nationality: representant.nationality,
                    idNumber: '', issuedDate: representant.issued_date, expiryDate: representant.expiry_date, countryOfBirth: representant.pays_naissance,
                    cityOfBirth: representant.ville_naissance, phoneNumber: representant.mobile_number, email: representant.email, businessSector: immatriculation.secteur_activite,
                    mainLineOfBusiness: immatriculation.activite_principale, atRate: '', region: immatriculation.reprise_region.designation,
                    department: immatriculation.reprise_departement.designation, arondissement: immatriculation.reprise_ville.designation,
                    commune: immatriculation.reprise_commune.designation, qartier: immatriculation.reprise_quartier.designation, address: immatriculation.adresse}
