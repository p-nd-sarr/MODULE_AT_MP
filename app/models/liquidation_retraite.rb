...
class LiquidationRetraite < ApplicationRecord
  include MyTools
  ...
  attr_accessor :dossier_cip_exist
  ...
  validates :numero_affiliation, :prenom, :nom, :date_naissance, :lieu_naissance, :date_cessation_activite,
            :type_retraite, :mode_paiement,
            presence: true
  ...
  def est_dossier_valide
    ...
    if participant.nil?
      halt! 'Ce dossier ne peut pas être validé : Le salarié est introuvable sur la base de Prestation.'
    end
  end
  ...
  def nb_jours_travail
    [
      carrieres_prestation.regime_general.valide.map(&:nb_jours_travail).sum,
      carrieres_prestation.regime_cadre.valide.map(&:nb_jours_travail).sum
    ].max
  end
  ...
  def calcul_date_jouissance
    ...
    unless cip_id.nil?
      dossier_cip = Cip.where(id: cip_id).first
      unless dossier_cip.nil?
        da = dossier_cip.created_at
      end
    end
    dj = da
    ...
    date_anniversaire = if retraite_normale?
                          if date_cessation_activite > Date.new(2016, 7, 1)
  ...
  def generate_allocataire!
    if dossier_valide? && !Allocataire.exists?(numero_allocataire: numero_affiliation)
      GenerateAllocataireFromLiquidationJob.perform_later(self)
    end
  end
    ...
    with_lock do
      if dossier_valide? && !Allocataire.exists?(numero_allocataire: numero_affiliation)
        ...
        if allocataire.save
          ...
          puts("Errorrr creation allocataire : ", allocataire.errors.messages)
          return allocataire
        end
      end
    end
  ...
  def set_numero_dossier!
    numero_ordre = LiquidationRetraite.where(numero_affiliation: numero_affiliation)
                                      .maximum(:num_dossier)
                                      .try(:split, '/')
                                      .try(:last)
                                      .try(:next) || 1
    ...
    self.num_dossier = "R/#{numero_affiliation}/#{annee}/#{numero_ordre}"
  end
